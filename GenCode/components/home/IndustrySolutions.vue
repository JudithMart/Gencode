  ...
  if (typeof window !== 'undefined') {
    ...
    if (carouselIndex.value + cardsPerView.value > cards.length) {
      carouselIndex.value = Math.max(
        0,
        cards.length - cardsPerView.value
      )
    }
  }
}
...
const visibleCards = computed(() => {
  ...
  return cards.slice(
    carouselIndex.value,
    carouselIndex.value + cardsPerView.value
  )
})
  ...
  <section class="bg-fondo-gray w-full py-12 sm:py-16 lg:py-20">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <p data-aos="fade-up" data-aos-delay="900" data-aos-duration="800" class="text-[#C7C8CA] mt-4 sm:mt-6 
        ...
        Contamos con amplia experiencia en diferentes sectores como, alimenticio, agrícola, inmobiliario, moda, arte,
        entre otros
      </p>
    </div>
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8 sm:mt-12">
      ...
      <button class="absolute left-0 sm:left-2 top-1/2 -translate-y-1/2 z-20 bg-[#222]/90 backdrop-blur-sm text-white rounded-full 
          ...
          disabled:opacity-30 disabled:hover:bg-[#222]/90 disabled:hover:shadow-black/50 hidden sm:flex  right-0"
        @click="prevSlide" :disabled="carouselIndex === 0" aria-label="Anterior">
      ...
      <div data-aos="fade-up" data-aos-delay="1000" data-aos-duration="800" class="overflow-x-auto sm:overflow-hidden scroll-smooth scrollbar-hide
        ...
        <div class="flex sm:grid gap-4 sm:gap-6 py-4 transition-all duration-500" :class="{
          ...
          <Card v-for="(card, i) in visibleCards" :key="carouselIndex + '-' + i + '-' + card.title" :image="card.image"
            :title="card.title" :text="card.text" :ruta="card.ruta" class="w-[80%] sm:w-full flex-shrink-0" />

        </div>
      </div>

      <button class="absolute right-0 sm:right-2 top-1/2 -translate-y-1/2 z-20 bg-[#222]/90 backdrop-blur-sm text-white rounded-full 
          w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center shadow-lg shadow-black/50 
          transition-all duration-300 hover:bg-primary hover:shadow-[0_0_15px_rgba(0,145,255,0.4)]
          disabled:opacity-30 disabled:hover:bg-[#222]/90 disabled:hover:shadow-black/50 hidden sm:flex "
        @click="nextSlide" :disabled="carouselIndex >= maxIndex" aria-label="Siguiente">
        <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- Texto final -->
    ...
    <div data-aos="fade-up" data-aos-delay="1000" data-aos-duration="800"
      ...
      <p class="tracking-wider text-center font-semibold bg-gradient-to-b from-text-fromGray to-text-toGray bg-clip-text text-transparent
        text-4xl sm:text-4xl md:text-7xl lg:text-7xl 2xl:text-7xl max-w-5xl leading-tight">
        Nuestros servicios
